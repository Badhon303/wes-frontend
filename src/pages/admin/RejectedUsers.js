...
export default function PendingUsers() {
    ...
    const getUsersApi = async (id) => {
        ...
        const data = await fetch(
            `${BASE_URL}/users?approvalStatus=rejected&role=user&page=${activePage}&sortBy=createdAt:desc`,
            {
                method: "GET",
                headers: myHeaders,
            }
    ...
    function transformRecord(obj) {
        ...
        if (obj.user.lastName)
            fullName = obj.user.lastName + " ";
        if (obj.user.firstName) fullName = fullName+ obj.user.firstName + " ";
        if (obj.user.middleName)
            fullName = fullName + obj.user.middleName + " ";

        return {
            record: obj,
            fullName: fullName,
            nickName: obj.user.nickName,
            introducerName: obj.introducer && obj.introducer.nickName &&  obj.introducer.email ?
                obj.introducer.email+'('+obj.introducer.nickName+')'  : '',
            introducerId:obj.introducer ? obj.introducer.id : null,
            email: obj.user.email,
            avatar: obj.user.photo && obj.user.photo !=='no-photo.jpg' ? PHOTO_URL +  obj.user.photo : AvatarLogo,
                        ...
                        {loading ? (
                            ...
                            <div className="bg-white shadow-md rounded my-6">
                                <ReactDataTable
                                    ...
                                    className="px-2  w-full h-full py-2  my-4 overflow-auto border-1"
                                    ...
                                    columns={[
                                        {
                                            key: "fullName",
                                            text: "Name",
                                            className: "text-black bg-white text-left px-2 w-40 break-words",
                                            align: "center",
                                            sortable: true,
                                        ...
                                        },
                                        {
                                            ...
                                            cell: (obj) => {
                                                return  <div
                                                    onClick={() => handleViewPendingUser(obj.introducerId)}
                                                ...
                                                return <div className="flex align-center justify-center">
                                                    <div className="mr-2">
                                                        <img alt="profile"
                                                             ...
                                                             src={obj.avatar !=="no-photo.jpg" ? obj.avatar : AvatarLogo}
                                                        />
                                                    </div>
                                                </div>
                                            }
                                        },
                                        {
                                            ...
                                            cell: (obj) => {
                                                return <span
                                                    className={
                                                        (obj.approvalStatus && obj.approvalStatus === "approved"
                                                            ? "bg-green-600"
                                                            : (obj.approvalStatus && obj.approvalStatus === "rejected" ? "bg-red-600" : "bg-yellow-600") )+
                                                        " text-white font-bold py-1 px-3 rounded-full text-xs"
                                                    }
                                                >
                          {obj.approvalStatus && obj.approvalStatus.charAt(0).toUpperCase() +  obj.approvalStatus.slice(1)}
                        </span>
                                            }
                                        },
                                        {
                                            ...
                                            align: "center",
                                            sortable: true,
                                            className: "text-gray-500 font-gibson text-base py-1 px-3 break-all ",
                                        },
                                        {
                                            key: "btcAddress",
                                            text: "BTC Address",
                                        ...
                                        },
                                        {
                                            ...
                                            sortable: true,
                                            cell: (obj) => {
                                                return <div className="flex item-center justify-center break-words">
                                                    <div
                                                        onClick={() => handleViewPendingUser( obj.record.user.id)}
                                                    ...
                                                    <div /* Edit */
                                                        onClick={() => {
                                                            history.push(`/user/${ obj.record.user.id}/edit`);
                                                        }}
                                                        className="w-6  cursor-pointer  mr-2 transform hover:text-purple-500 hover:scale-110"
                                                    >
                                                        <svg
