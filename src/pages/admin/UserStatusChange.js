...
export default function UserStatusChange(props) {
  ...
  const approveUser = async (status) => {
    ...
    const data = await fetch(
      `${BASE_URL}/users/${userId}/change-approval-status`,
      {
        method: "PUT",
        body: JSON.stringify({
  ...
  const getUserInfo = async (userId) => {
    ...
    if (response) {
      ...
      if (response.code === 401) history.push("/signin")
        ...
        setApproveStatus(
          response.user.approvalStatus &&
            response.user.approvalStatus.charAt(0).toUpperCase() +
              response.user.approvalStatus.slice(1)
        )
      }
    } else console.log("Whoops..", "No user data found", "error")
  }
            ...
            <div className=''>
              <div className='my-4 bg-gray-50 px-4 py-4 border-1 overflow-hidden sm:rounded-md'>
                <div className=' my-4 mx-2 '>
                  <p className='text-xl text-left w-64'>
                    {" "}
                    Present Status
                    <span
                      className={
