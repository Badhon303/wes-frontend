...
function BTAccountSuccess({btc,ether,hideLogin}) {
   ...
   const  downloadBitcoin = () => {
    ...
    const  downloadEther= () => {
        const element = document.createElement("a");
        const file = new Blob([etherAddress],
            {type: 'text/plain;charset=utf-8'});
        element.href = URL.createObjectURL(file);
    ...
    return (
        <div
            ...
            <div
                className="py-6 px-3 md:px-0 lg:px-0 xl:px-0 lg:w-70 sm:w-full md:w-90 xl:w-70 items-center justify-center md:mx-auto  ">
                <div className="mx-auto p-4  mt-12 bg-white  border-1  border-gray-400 rounded-2xl">
                    ...
                    <div className="px-6 py-4">
                        ...
                        <div
                            className=" mt-2 bg-white shadow border-1 flex rounded-l-lg  rounded-r-0 w-full flex  flex-row">
                            <div
                                className="text-gradians md:text-base text-sm px-6  font-bold py-2 word-break justify-self-start break-all "> {etherAddress} </div>
                            <div onClick={downloadEther}
                                ...
                                <DownloadIcon/></div>
                            <CopyToClipboard text={etherAddress}
                        ...
                        <p className="pt-6 text-title text-md font-medium font-gibson">Bitcoin</p>

                        <div
                            className=" mt-2 bg-white shadow border-1 flex rounded-l-lg  rounded-r-0 w-full flex  flex-row">
                            <div
                                className="text-gradians md:text-base text-sm px-6  font-bold py-2 word-break justify-self-start break-all"> {bitcoinAddress} </div>
                            <div onClick={downloadBitcoin}
                                className="ml-auto   flex  items-center px-3 bg-site-theme outline-none focus:outline-none shadow focus:shadow-xl hover:shadow cursor-pointer ">
                                <DownloadIcon/></div>
                        ...
                        {!hideLogin  &&   <div className="mt-4  flex justify-center">
                            <div>
                                <span className="text-base pr-2  ">Already have account?</span> <Link
