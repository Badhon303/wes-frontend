...
export default function UserOrderPage() {
    ...
    const getUserApi = async () => {
        ...
        if (userInfo && Cookies.get('access-token')) {
            ...
            if (response) {
                ...
                else if (response.code === 404 || response.code === 403 ||  response.code === 500) Swal.fire('Whoops..', "No user data found", 'error');
                else {
    ...
    const callOrderApi = async () => {
        ...
        const data = await fetch(`${BASE_URL}/purchase/orders?sortBy=createdAt:desc&page=${activePage}&limit=5`, {
            method: "GET",
            headers: myHeaders,
        });
        const response = await data.json();
        ...
        if (response) {
            setLoading(false)
            if (response.code === 401) history.push('/signin');
            else if (response.code === 404 || response.code === 403 ||  response.code === 500) Swal.fire('Whoops..', "No orders found", 'error');
            else {
            ...
            <main className="flex justify-center ">
                ...
                <div className="lg:max-w-7xl w-full bg-white rounded-xl p-2 md:p-6">
                    <div className="rounded-xl border-1  w-full rounded-xl px-2 md:px-16 py-2 md:py-6">
                        ...
                        {orders && orders.length && orders.filter(obj => obj.length > 0).map((result) =>
                            (<div className="bg-white  w-full my-2 border-1 block shadow hover:shadow-xl">
                                    ...
                                    <div
                                        ...
                                        <div>
                                            <p className="text-left group-hover:underline cursor-pointer group-hover:text-site-theme md:font-medium" onClick={()=> handleOrderDetails(result[0])}>{result && result.length && result && result[0] && result[0].order_id}</p>
                                            <p className="text-gray-400 md:text-sm  text-10px">{result[0].ordered_at && getReadableTime(result[0].ordered_at)}</p>
                                        </div>
                                        <div>
                                            {result[0].items && result[0].items.map((obj) => (
                                                <div>
                                                    <p className="text-left text-10px md:text-sm text-gray-900">{obj && obj.item_id}   : <span className="ml-1">{obj && obj.unit_quantity} Unit</span></p>

                                                </div>
                                            ))}
                                        </div>
                                        {result && result.length && result && result[0] && result[0].status === "TOKEN_SEND" &&

                                        <button
                                            className="rounded bg-gray-600 p-1 text-center bg-green-600 px-4 text-white text-10px md:text-base">Successful</button>
                                        }
                                        {result && result.length && result && result[0] && result[0].status === "CONFIRMED_PAYMENT_RECEIVED" &&

                                        <button
                                            className="rounded bg-gray-600 p-1 text-center bg-site-theme px-4 text-white text-10px md:text-base">Pending</button>
                                        }
                                        {result && result.length && result && result[0] && (result[0].status !== "TOKEN_SEND" && result[0].status !== "CONFIRMED_PAYMENT_RECEIVED") &&
