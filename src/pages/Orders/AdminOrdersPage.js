...
export default function AdminOrdersPage() {
  ...
  const callRewardHistoryApi = async (orderId) => {
    ...
    const data = await fetch(
      `${BASE_URL}/referral-point/reward-history?orderId=${orderId}`,
      {
        method: "GET",
        headers: myHeaders,
      }
          ...
          <div className='rounded-xl border-1  w-full rounded-xl px-2 md:px-16 py-2 md:py-6'>
            ...
            <div className='bg-white  w-full my-2 border-1 block shadow '>
              <div className='flex flex-row justify-between px-4 py-4 items-center text-sm md:text-base group'>
                ...
                <div className='text-left text-10px md:text-sm text-gray-900 font-semibold'>
                  Introducers
                </div>
              </div>
            </div>
                  ...
                  <div className='flex flex-row justify-between px-4 py-4 items-center text-sm md:text-base group'>
                    <div>
                      ...
                      <p className='text-gray-400 md:text-sm  text-10px break-all'>
                        {result.ordered_at &&
                          getReadableTime(result.ordered_at)}
                      </p>
                    </div>
                    ...
                    {result && result.status === "TOKEN_SEND" && (
                      <div className='rounded bg-gray-600 p-1 text-center bg-green-600 px-4 text-white text-10px md:text-base'>
                        Successful
                      </div>
                    )}
                    {result &&
                      result.status === "CONFIRMED_PAYMENT_RECEIVED" && (
                        <div className='rounded bg-gray-600 p-1 text-center bg-site-theme px-4 text-white text-10px md:text-base'>
                          Pending
                        </div>
                      )}
                    {result &&
                    ...
                    <button
                      onClick={() => handleIntroducersModal(result.orderId)}
