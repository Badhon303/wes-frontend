...
export default function ReferralPurchaseUserPage() {
  ...
  const validate = (values) => {
    ...
    if (!refPoint) {
      ...
      errors.refPoint =
        "Amount should be multiplication of 100 like, 200,300..."
    } else if (referralPoint && refPoint > referralPoint.totalPoint) {
      errors.refPoint =
        "Amount must not be greater then your total referral point"
    }
  ...
  const getUserReferralStatus = async () => {
    if (userInfo && Cookies.get("access-token")) {
      ...
      const data = await fetch(
        `${BASE_URL}/referral-point/exchange/check-status`,
        {
          method: "GET",
          headers: myHeaders,
        }
  ...
  const getDollarPriceApi = async () => {
    if (userInfo && Cookies.get("access-token")) {
      ...
      const data = await fetch(
        `${BASE_URL}/referral-point/dollar-price?amount=${formik.values.refPoint}`,
        {
          method: "GET",
          headers: myHeaders,
        }
      ...
      if (response) {
        if (response.code === 401) history.push("/signin")
          ...
          Swal.fire("Whoops..", "No price info found", "error")
        else {
  ...
  const getAmountOfCurrencyApi = async (dollarPrice, token) => {
    if (userInfo && Cookies.get("access-token")) {
      ...
      const data = await fetch(
        `${BASE_URL}/tokens/amount?token=${token}&dollar=${dollarPrice}`,
        {
          method: "GET",
          headers: myHeaders,
        }
  ...
  return (
    <div>
      ...
      <main className='md:flex md:justify-center '>
        {loading === false && (
          <div className='lg:max-w-7xl w-full bg-white rounded-xl p-2 md:p-6 container'>
            <div className=' rounded-xl border-1  w-full rounded-xl px-2 md:px-16 py-2 md:py-12'>
              <p className='text-base text-left font-bold py-4 text-gray-700'>
                Available Referral Point:{" "}
                {referralPoint &&
                  referralPoint.totalPoint &&
                  referralPoint.totalPoint}{" "}
              </p>
              ...
              referralStatus.isEligible === true ? (
                ...
                <p className='text-md font-bold my-4'>
                  Sorry Your are not eligible for referral point transfer{" "}
                </p>
              )}
                ...
                <div>
                  <table className='w-full border border-outline-color container'>
                    <thead className='border border-outline-color bg-site-theme text-white'>
                      <tr>
                        <th className='border-r-1 border-outline-color'>
                          Title
                        </th>
                        <th>Status</th>
                      </tr>
                    ...
                    <tbody className='border border-outline-color'>
                      <tr className='border-t  border-outline-color'>
                        <td className='px-2 border-r border-outline-color'>
                          Purchased (at least once)
                        </td>
                        <td className='px-2 text-center'>
                          {referralStatus.hasPurchased === true ? (
                      ...
                      <tr className='border-t  border-outline-color'>
                        <td className='px-2 border-r border border-outline-color'>
                          Has Adequate Referral Point
                        </td>
                        <td className='px-2 text-center'>
                          {referralStatus.hasEnoughPoints === true ? (
                            <FontAwesomeIcon icon={faCheck} />
                          ) : (
                            <FontAwesomeIcon icon={faTimes} />
                          )}
                      ...
                      <tr className='border-t border-outline-color'>
                        ...
                        <td className='px-2 text-center'>
                          {referralStatus.isApproved === true ? (
                            <FontAwesomeIcon icon={faCheck} />
                          ) : (
                            <FontAwesomeIcon icon={faTimes} />
                          )}
                    ...
                    <table className='w-full border border-black'>
                      ...
                      <tbody className='border border-black'>
                        ...
                        <tr>
                          ...
                          <td className='px-2 text-center'>
                            {formik.values.refPoint}
                            <span>(USD{purchaseData.dollarPrice})</span>
                          </td>
                        </tr>
                        <tr className='border-b border-black'>
                          <td className='px-2 border-r border-black'>
                            Referral Points Available after Purchase
                          </td>
                          <td className='px-2 text-center'>
                            {calcReferralAmount()}
                          </td>
