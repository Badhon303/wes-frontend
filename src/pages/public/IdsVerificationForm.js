...
export default function IdVerificationForms(props) {
    ...
    return(
        ...
        <div className="   ">
            ...
            <div>
                <p className="text-lg font-bold text-left text-gray-500 pb-4">Id Verification </p>

                <p className=" text-base font-medium text-left py-2 text-gray-400 text-base">Select document type </p>

                <form onSubmit={formik.handleSubmit}>
                    <div className=" flex flex-col  ">
                        <div onClick={() => setId(1)}
                             className={"my-2 cursor-pointer shadow border-2 font-bold hover:bg-site-theme hover:text-white focus:bg-site-theme focus:text-white  text-center py-4 " + (id === 1 ? 'bg-site-theme text-white' : 'bg-white text-back')}>

                            <span className="flex justify-start ">{id === 1 &&
                            <img src={tickSign} className="h-6 w-6 text-center mx-4 " alt="tick sign"/>} <p
                                className={id === 1 ? 'pl-0' : 'pl-14'}>  ID Card  </p></span>
                        </div>
                        <div onClick={() => setId(2)}
                             className={"my-2 cursor-pointer shadow border-2 font-bold hover:bg-site-theme hover:text-white focus:bg-site-theme focus:text-white  text-center py-4 " + (id == 2 ? 'bg-site-theme text-white' : 'bg-white text-back')}>
                            <span className="flex justify-start ">{id === 2 &&
                            <img src={tickSign} className="h-6 w-6 text-center mx-4 " alt="tick sign"/>}  <p
                                className={id === 2 ? 'pl-0' : 'pl-14'}>  Passport </p></span>

                        </div>
                        <div onClick={() => setId(3)}
                             className={"my-2 cursor-pointer shadow border-2 font-bold hover:bg-site-theme hover:text-white focus:bg-site-theme focus:text-white  text-center py-4 " + (id === 3 ? 'bg-site-theme text-white' : 'bg-white text-back')}>
                            <span className="flex justify-start ">{id === 3 &&
                            <img src={tickSign} className="h-6 w-6 text-center mx-4 " alt="tick sign"/>} <p
                                className={id === 3 ? 'pl-0' : 'pl-14'}>   Driving Licence </p>  </span>
            ...
            {
                id === 1 && verificationType === 1   &&  <NID
                    userInfo={props.userInfo}
                    cb={onVerificationDone} />
            }

            {
                id === 2 && verificationType === 2 &&  <Passport
                    userInfo={props.userInfo}
                    cb={onVerificationDone} />
            }

            {
                id === 3 && verificationType === 3 && <DrivingLicense
                    userInfo={props.userInfo} cb={onVerificationDone} />
            }
