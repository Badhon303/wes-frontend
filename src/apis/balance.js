...
const getBalance = async (address, name, type = 'erc20') => {
  ...
  return sendGetRequest(url, null, { credential: true })
    .then(response => {
      if (response) {
        if (response.data.status_code === 401) window.location.href = '/signin'; // TODO:: move this to FetchApi.js and instead of redirection should refresh token
        else if (response.data.status_code === 404 || response.data.status_code === 500) console.log('Whoops..', "No balance found", 'error');
