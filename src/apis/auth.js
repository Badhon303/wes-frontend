...
export function signup(data, referred) {
  if (referred)
    return sendPostRequest("/auth/register?referrerId=" + referred, data)
  else return sendPostRequest("/auth/register", data)
}

export function OTP(data) {
  return sendPostRequest("/auth/match-otp", data)
}
...
export function changePass(userId, data) {
  return sendPutRequest(`/users/${userId}/change-password`, data, {
    credential: true,
  })
}
...
export function emailVerify(data) {
  return sendPostRequest("/auth/register/email-verification", data)
}
